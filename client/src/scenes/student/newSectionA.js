...
class NewSectionA extends Component {
  ...
  handleSubmit = () => {
    alertConfirm({
      title: 'Confirmation',
      content:
        'Are you sure you want to submit this section? Once submitted you will not be able to return back to section A.',
      okText: 'Yes',
      cancelText: 'No',
      onOk: () => {
        // Getting time from the Timer component
    ...
    Axios.get(
      ...
      res => {
        if (res.data.data.count.mcq > 0) {
          ...
          var os = [];
          var selected = [
            ...res.data.data.submittedAnswers.responses,
          ];
