...
class StudentList extends Component {
  ...
  handlePrint = () => {
    ...
    Axios.get(
      `/api/generate/pdf?year=${this.state.year}&type=${this.state.type}&div=${this.state.div}`,
      {
        headers: {
      ...
      .then((res) => {
        ...
        saveAs(blob, `results.tar.gz`);
      })
      .catch((e) => console.log(e));
  };
  handleExcel = () => {
    Axios.get(
      `/api/generate/excel?year=${this.state.year}&type=${this.state.type}&div=${this.state.div}`,
      {
        headers: {
          Authorization: localStorage.getItem('token'),
        },
      ...
      (err) =>
        alert(
          'Please make sure that all the dropdowns have a valid input',
        ),
    );
  };
  search = (event) => {
  ...
  getStudentsList = () => {
    Axios.get(
      `/api/evaluate/responses?year=${this.state.year}&type=${this.state.type}&div=${this.state.div}`,
      {
        headers: {
          Authorization: localStorage.getItem('token'),
        },
      ...
      this.setState({
        ...
        value: {
          min:
            ...
            parseInt(minSapId),
          max:
            parseInt(localStorage.getItem('max')) ||
            parseInt(maxSapId),
        },
  ...
  render() {
    ...
    const renderCard = this.state.filter
      .filter((s) => {
        return (
          parseInt(s.sapId) >= this.state.value.min &&
          parseInt(s.sapId) <= this.state.value.max
        );
      })
      .map((s) => {
        delay = delay + 50;
        correctedCount = s.print
          ? correctedCount + 1
          : correctedCount;
        return (
          <Grid item xs={12}>
        ...
        <Grid container direction="row" justify="center">
          <Grid item xs={10}>
            ...
            >
              {/* Select Year */}
              <Grid item xs={4}>
                <InputLabel style={{ marginLeft: 10, marginTop: 50 }}>
                  {' '}
                  Year{' '}
                </InputLabel>
                <Select
              ...
              <Grid style={{ marginBottom: 40 }} item xs={12}>
                {this.state.rangeMax > 0 &&
                this.state.rangeMax - this.state.rangeMin > 5 ? (
                  <React.Fragment>
                    <div
                      style={{
                        letterSpacing: 1,
                    ...
                    >
                      {' '}
                      <b>Corrected Papers : {correctedCount}</b>{' '}
                    ...
                    >
                      Note : The slider provided below can be used to
                      filter the range of SAP IDs
                    </div>
                    <div
                      style={{
                        marginBottom: 30,
